...
export function SystemClock() {
  ...
  // BOLT OPTIMIZATION: Only run high-frequency updates when the popover is actually visible.
  useEffect(() => {
    if (!isOpen) return;

    // Immediately sync time on open to avoid up to 10s of staleness from the pill interval
    setTime(new Date());
