...
export const orphanedRecordDetectionFn: InngestFunction.Any =
      ...
      try {
        ...
        if (invalidMachineOps && invalidMachineOps.length > 0) {
          issues.push({
            ...
            details: invalidMachineOps.map(
              (op) => `ID: ${op.id}, Machine ID: ${op.machine_id}`,
            ),
          });
        ...
        const { data: orphanedLoads, error: loadsError } = await serviceRole
          ...
          .not(
            "machine_id",
            "in",
            serviceRole.from("machines").select("id").eq("active", true),
          );
        ...
        if (orphanedLoads && orphanedLoads.length > 0) {
          issues.push({
            ...
            details: orphanedLoads.map(
              (load) =>
                `ID: ${load.id}, Machine ID: ${load.machine_id}, Date: ${load.load_date}`,
            ),
          });
        ...
        if (invalidOps && invalidOps.length > 0) {
          issues.push({
            ...
            details: invalidOps.map(
              (op) => `ID: ${op.id}, Operator ID: ${op.operator_id}`,
            ),
          });
        ...
        const { data: invalidShiftStatus, error: shiftError } =
          await serviceRole
            .from("shift_status")
            ...
            .not(
              "department_id",
              "in",
              serviceRole.from("departments").select("id"),
            );
