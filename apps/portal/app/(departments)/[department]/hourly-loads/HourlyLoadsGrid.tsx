...
export function HourlyLoadsGrid({
  ...
  // Memoize loadsByMachine map creation using a composite key: machine_id:shift_type
  // This avoids O(N) recreations during renders and prevents day/night shift overwrites.
  ...
  const handleCellChange = useCallback(
    async (rowIndex: number, hourProp: string, delta: number) => {
      ...
      await trackClientMetric(
        ...
        async () => {
          try {
            // Optimize array searching (O(N)) with O(1) Map lookup
