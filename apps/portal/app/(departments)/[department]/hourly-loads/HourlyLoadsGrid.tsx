...
export function HourlyLoadsGrid({
  ...
  // Performance Optimization: Memoize the derived Map structure with a composite key (machine_id:shift_type).
  // This avoids recreating the Map on every render, stabilizing callback references, preventing cascading re-renders,
  // and ensuring day and night shifts don't overwrite each other.
  ...
  const handleCellChange = useCallback(
    async (rowIndex: number, hourProp: string, delta: number) => {
      ...
      await trackClientMetric(
        ...
        async () => {
          try {
            // Performance Optimization: Replace O(N) find with O(1) Map lookup
