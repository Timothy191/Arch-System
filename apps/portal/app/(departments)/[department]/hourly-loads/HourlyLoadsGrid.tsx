...
export function HourlyLoadsGrid({
  ...
  const loadsByMachine = useMemo(() => {
    // Performance Pattern: Memoize derived data structures to stabilize dependent useCallback / useMemo hooks
    const map = new Map<string, HourlyLoad>();
    hourlyLoads.forEach((load) => {
      // Use composite key of machine_id:shift_type to prevent day/night shift records overwriting each other
  ...
  const handleCellChange = useCallback(
    async (rowIndex: number, hourProp: string, delta: number) => {
      ...
      await trackClientMetric(
        ...
        async () => {
          try {
            // Performance Pattern: Replace O(N) array searching with O(1) Map lookup
