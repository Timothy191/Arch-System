...
export function ExcavatorActivityList({
  ...
  // AGENT-TRACE: Pre-index todayAssignments into a Map keyed by excavator_activity_id.
  // Replaces O(N * M) repeated .filter() calls during render iterations with O(1) Map lookups.
  const assignmentsByActivityId = useMemo(() => {
    ...
    for (const assignment of todayAssignments) {
      ...
      map.set(assignment.excavator_activity_id, list);
    }
    return map;
  }, [todayAssignments]);
