...
export function ExcavatorActivityList({
  ...
  // Pre-index dumper assignments by excavator_activity_id for O(1) lookups
  // Performance Pattern: Replaces repeated O(N) .filter calls per activity with O(1) Map lookup
    ...
    <div className="space-y-6">
      ...
      {siteEntries.map(([siteKey, { siteName, activities }]) => {
        const siteAssignments = activities.flatMap(
          (a) => assignmentsByActivityId.get(a.id) ?? EMPTY_ASSIGNMENTS
        );
                ...
                <div className="space-y-2">
                  {dayOps.map((activity) => (
                    <ActivityCard
                      key={activity.id}
                      activity={activity}
                      assignments={assignmentsByActivityId.get(activity.id) ?? EMPTY_ASSIGNMENTS}
                    />
                  ))}
                </div>
              </div>
