...
export function ExcavatorActivityList({
  ...
  // Pre-index dumper assignments by excavator_activity_id into a Map to replace O(N * M) nested .filter() calls with O(1) Map lookups.
  const assignmentsByActivityId = useMemo(() => {
    ...
    for (const assignment of todayAssignments) {
      ...
      if (list) {
        ...
        map.set(activityId, [assignment]);
      }
    }
    return map;
  }, [todayAssignments]);

  // Group activities by site_id, memoized to avoid re-grouping and sorting on parent re-renders.
