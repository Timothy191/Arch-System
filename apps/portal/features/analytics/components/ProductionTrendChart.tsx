...
function ProductionTrendChartComponent({ data, showForecast = true }: ProductionTrendChartProps) {
  // AGENT-TRACE: Memoize linear forecasting calculations and chart dataset construction.
  // Prevents re-executing forecasting algorithms and creating new array references on parent re-renders,
  // avoiding unnecessary SVG AreaChart re-diffing.
  const { chartData, categories, hasForecast } = useMemo(() => {
    if (data.length === 0) {
      return { chartData: [], categories: [], hasForecast: false };
    }
...
// AGENT-TRACE: Memoize ProductionTrendChart — maintains component referential equality
// across parent re-renders when props remain unchanged.
