...
- Official Turborepo docs: <https://turbo.build/reference/docs>
- Official Next.js docs: <https://nextjs.org/docs>
- Official pnpm docs: <https://pnpm.io/docs>

### Required Improvements

> **Superseded by the Nx → Turborepo migration.** The monorepo migrated from Nx to Turborepo (see `docs/wiki/concepts/adr-011-turborepo-monorepo.md`). The former Nx-specific items below are no longer applicable and are resolved by the Turbo setup:

#### 1.1 Nx plugins (`@nx/next/plugin`, `@nx/eslint/plugin`) — N/A

- **Status**: Nx removed entirely. Next.js runs on Turbopack; linting uses `eslint-plugin-boundaries` (generated by `pnpm policy:gen` from `tools/policy-compiler.cjs`).

#### 1.2 Dependency constraints — Resolved

- **Status**: Enforced via `eslint-plugin-boundaries` (`scope:*` tags) rather than Nx `dependencyConstraints`. See `tools/policy-compiler.cjs`.

#### 1.3 Named inputs for test targets — Resolved

- **Status**: Per-task `inputs` configured in `turbo.json` (e.g. `test` excludes `!**/*.test.*` from build inputs).

#### 1.4 CI-specific task runner config — Resolved

- **Status**: `turbo.json` `globalDependencies` + `globalEnv` cover CI; affected runs use `--filter=...[origin/main]`.

#### 1.5 `project.json` per package — N/A

- **Status**: Turborepo reads task config from each package's `package.json`; no `project.json` files are used.
...
1. ~~Add `@nx/eslint/plugin` to `nx.json`~~ (superseded — Nx removed; boundaries via `eslint-plugin-boundaries`)
