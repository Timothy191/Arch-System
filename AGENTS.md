...
Nx + pnpm monorepo: Next.js 15 portal (`apps/portal`), Payload CMS (`apps/cms`), React Flow (`apps/overview`), and `@repo/*` packages. CI quality gates are strict and non-negotiable.
...
- Migrations are zero-padded SQL files in `packages/database/migrations/` (e.g. `062_add_table.sql`), applied lexically. Commit migration + regenerated `packages/supabase/src/database.types.ts` as one atomic change.
...
- `apps/*` and `packages/*` are workspace roots; no packages outside standard layout need `pnpm-workspace.yaml` edits.
- Shared dependencies use `catalog:` or `catalog:react19` indirection (see `pnpm-workspace.yaml`).

## Nx specifics

- Build depends on `^build` + `^codegen` + `sync-assets`. The `sync-assets` target copies `workspaceRoot/shared/` into `portal/public/`.
...
- Next.js 15 + Turbopack: `turbopack.root` is set to workspaceRoot so `packages/` deps compile.
