...
- [ ] **Setup Local DB Stack**: Ensure Docker is active and start the local Supabase instance.
  ...
  ```
- [ ] **Create Migration File**: Generate a new zero-padded migration SQL file under `01_platform_packages/database/migrations/` (e.g. `063_add_feature_table.sql`).
...
Use this checklist when creating a reusable platform utility under `01_platform_packages/`.

- [ ] **Initialize Package Directory**: Create the package folder, e.g. `01_platform_packages/payment-gateway/`.
...
- [ ] **If imported by portal test**: Add a corresponding `moduleNameMapper` mapping entry inside `00_applications/portal/jest.config.js`.

---

## 3. Implementing a New Feature Module (Domain Separation)

Use this checklist when developing features under `02_domain_libraries/features/[feature]/` to ensure strict frontend/backend separation.
