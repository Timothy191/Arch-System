...
test.describe("department navigation", () => {
  ...
  test("unauthenticated access to department tab redirects with full path", async ({ page }) => {
    await page.goto("/drilling/machines");
    await expect(page).toHaveURL(/\/login/);
    expect(page.url()).toContain("redirect=%2Fdrilling%2Fmachines");
  });

  test("unauthenticated access to daily-log redirects with full path", async ({ page }) => {
    await page.goto("/production/daily-log");
    await expect(page).toHaveURL(/\/login/);
    expect(page.url()).toContain("redirect=%2Fproduction%2Fdaily-log");
  });

  test("unauthenticated access to breakdowns redirects with full path", async ({ page }) => {
...
test.describe("login page navigation", () => {
  ...
  test("direct navigation to /login preserves any redirect param", async ({ page }) => {
...
test.describe("404 and error handling", () => {
  test("unknown route still redirects to login when unauthenticated", async ({ page }) => {
