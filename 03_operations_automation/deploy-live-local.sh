...
if [ ! -f "$ENV_FILE" ]; then
  info "Seeding 00_applications/portal/.env from 00_applications/portal/.env.example..."
  cp "$PORTAL_DIR/.env.example" "$ENV_FILE"
fi
if [ ! -f "$REPO_ROOT/.env" ]; then
  info "Seeding root .env from 00_applications/portal/.env.example..."
  cp "$PORTAL_DIR/.env.example" "$REPO_ROOT/.env"
fi

python3 "$REPO_ROOT/03_operations_automation/ensure_reachability.py" "$selected_ip" "$anon_key" "$service_key"
...
if [ -f "$REPO_ROOT/10_infrastructure_as_code/docker/compose.tools.yml" ]; then
  $COMPOSE_CMD -f "$REPO_ROOT/10_infrastructure_as_code/docker/compose.tools.yml" up -d
fi

if [ -f "$REPO_ROOT/10_infrastructure_as_code/14_observability_configuration/docker-compose.yml" ]; then
  $COMPOSE_CMD -f "$REPO_ROOT/10_infrastructure_as_code/14_observability_configuration/docker-compose.yml" up -d
fi
