...
# Usage: ./03_operations_automation/preflight-checklist.sh [--fix]
#
...
[ -d "$PORTAL_DIR" ]     && check_pass "00_applications/portal"   || check_fail "00_applications/portal missing"
[ -d "$DATABASE_DIR" ]   && check_pass "01_platform_packages/database" || check_fail "01_platform_packages/database missing"
...
if [ -f "$PORTAL_DIR/.env" ]; then
  check_pass "00_applications/portal/.env exists"
  ...
  check_fail "00_applications/portal/.env missing"
...
if [ -d "$PORTAL_DIR/.next/standalone" ]; then
  ...
  if [ -f "$PORTAL_DIR/.next/standalone/00_applications/portal/server.js" ]; then
...
SERVICE_FILE="$HOME/.07_toolchain_configuration/10_infrastructure_as_code/systemd/user/arch-systems.service"
if [ -f "$SERVICE_FILE" ]; then
  check_pass "Service file exists"
  if grep -q "standalone/00_applications/portal/server.js" "$SERVICE_FILE"; then
    check_pass "ExecStart uses standalone server.js"
  else
    check_fail "ExecStart does NOT use standalone server.js — fix required"
    if [ "$FIX_MODE" = true ]; then
      NODE_BIN=$(command -v node)
      sed -i "s|ExecStart=.*|ExecStart=$NODE_BIN $REPO_ROOT/00_applications/portal/.next/standalone/00_applications/portal/server.js|" "$SERVICE_FILE"
...
CLEAN_TARGETS=(
  "$REPO_ROOT/.nx/cache"
  "$REPO_ROOT/00_applications/portal/.next/cache"
  "$REPO_ROOT/00_applications/cms/.next/cache"
  "$REPO_ROOT/00_applications/overview/.next/cache"
)
