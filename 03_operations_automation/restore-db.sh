...
# Usage: ./03_operations_automation/restore-db.sh <timestamp>
#   e.g.  ./03_operations_automation/restore-db.sh 2026-06-16_143022
# ──────────────────────────────────────────────────────────

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BACKUP_DIR="$REPO_ROOT/03_operations_automation/backups"
...
resolve_db_url() {
  ...
  for env_file in "$REPO_ROOT/.env" "$REPO_ROOT/00_applications/portal/.env" "$REPO_ROOT/00_applications/portal/env/.env"; do
    if [ -f "$env_file" ]; then
      local candidate
