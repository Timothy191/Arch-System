...
# Usage: ./03_operations_automation/deploy.sh [local|production|staging] [options]
#
# Features:
...
PORTAL_DIR="$REPO_ROOT/00_applications/portal"
DATABASE_DIR="$REPO_ROOT/01_platform_packages/database"
SUPABASE_DIR="$REPO_ROOT/01_platform_packages/supabase"
      ...
      if [ ! -f "$PORTAL_DIR/.env" ] && [ ! -f "$PORTAL_DIR/.env.local" ]; then
        if [ -f "$PORTAL_DIR/.env.example" ]; then
          ...
          if grep -q -E "your-|TODO|CHANGEME" "$PORTAL_DIR/.env" 2>/dev/null; then
            warn "Created .env file contains placeholder secrets — verify 00_applications/portal/.env"
          fi
        else
...
phase_validate() {
    ...
    local)
      if [ ! -f "$PORTAL_DIR/.env" ] && [ ! -f "$PORTAL_DIR/.env.local" ]; then
        if [ -f "$PORTAL_DIR/.env.example" ]; then
          ...
          success "Created .env file from template"
          if grep -q -E "your-|TODO|CHANGEME" "$PORTAL_DIR/.env" 2>/dev/null; then
            warn "Created .env file contains placeholder secrets (e.g. your-...). Please verify keys in 00_applications/portal/.env"
          fi
        else
  ...
  run_if_not_dry rm -rf "$REPO_ROOT"/00_applications/portal/.next/cache "$REPO_ROOT"/00_applications/cms/.next/cache "$REPO_ROOT"/00_applications/overview/.next/cache "$REPO_ROOT"/01_platform_packages/eval/.pytest_cache
...
phase_stop_services() {
  ...
  if [ "$CLEAN_ONLY" = true ] || [ "$DEPLOY_MODE" = "local" ]; then
    local tools_compose="$REPO_ROOT/10_infrastructure_as_code/docker/compose.tools.yml"
    local monitor_compose="$REPO_ROOT/10_infrastructure_as_code/14_observability_configuration/docker-compose.yml"
...
phase_start_infrastructure() {
  ...
  case "$DEPLOY_MODE" in
    ...
    production|staging)
      local tools_compose="$REPO_ROOT/10_infrastructure_as_code/docker/compose.tools.yml"
      local prod_compose="$REPO_ROOT/10_infrastructure_as_code/docker/compose.production.yml"
...
echo "  Stop:      ./03_operations_automation/deploy.sh local --clean"
