...
# Usage: ./03_operations_automation/setup-production-environment.sh [options]
#
...
show_rocky_linux_guidance() {
  if [ "$OS" = "rocky" ] || [ "$OS" = "rhel" ] || [ "$OS" = "centos" ]; then
    ...
    echo
    echo -e "${BOLD}See: ${CYAN}03_operations_automation/ROCKY_LINUX_COMPATIBILITY.md${NC} for detailed guide"
    echo
...
setup_docker_tools() {
  ...
  local compose_file="$REPO_ROOT/10_infrastructure_as_code/docker/compose.tools.yml"
...
setup_monitoring() {
  ...
  local monitoring_file="$REPO_ROOT/10_infrastructure_as_code/14_observability_configuration/docker-compose.yml"
...
print_summary() {
  ...
  if [ "$OS" = "rocky" ] || [ "$OS" = "rhel" ] || [ "$OS" = "centos" ]; then
    ...
    echo -e "  • Compatibility guide: ${CYAN}03_operations_automation/ROCKY_LINUX_COMPATIBILITY.md${NC}"
