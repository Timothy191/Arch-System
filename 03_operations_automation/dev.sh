...
PORTAL_ENV_FILE="$REPO_ROOT/00_applications/portal/.env"
PORTAL_ENV_LOCAL="$REPO_ROOT/00_applications/portal/.env.local"
...
REDIS_COMPOSE="$REPO_ROOT/12_distributed_cache_runtime/docker-compose.yml"
...
if [ -f "$REPO_ROOT/03_operations_automation/sync-assets-smart.cjs" ]; then
  node "$REPO_ROOT/03_operations_automation/sync-assets-smart.cjs"
elif [ -f "$REPO_ROOT/03_operations_automation/sync-assets.sh" ]; then
...
source_files_stale() {
  ...
  [ ! -f "$marker" ] && return 0
  find "$REPO_ROOT/00_applications/portal" \
...
if [ "$FORCE_RESTART" = "true" ]; then
  ...
  clean_dir_cache "$REPO_ROOT/00_applications/portal/.next/cache" "Next.js portal cache"
  clean_dir_cache "$REPO_ROOT/00_applications/cms/.next/cache" "Next.js CMS cache"
  clean_dir_cache "$REPO_ROOT/00_applications/overview/.next/cache" "Next.js overview cache"
...
if [ ! -f "$PORTAL_ENV_FILE" ] && [ ! -f "$PORTAL_ENV_LOCAL" ]; then
  if [ -f "$REPO_ROOT/00_applications/portal/env/.env.example" ]; then
    echo -e "  ${INFO} Apps portal .env missing. Copying from env/.env.example..."
    cp "$REPO_ROOT/00_applications/portal/env/.env.example" "$PORTAL_ENV_FILE"
    check "Environment file" "pass" "copied from template"
    if grep -q -E "your-|TODO|CHANGEME" "$PORTAL_ENV_FILE" 2>/dev/null; then
      check "Environment secrets" "warn" "contains placeholder values — please configure them in 00_applications/portal/.env"
    fi
  elif [ -f "$REPO_ROOT/00_applications/portal/.env.example" ]; then
    echo -e "  ${INFO} Apps portal .env missing. Copying from .env.example..."
    cp "$REPO_ROOT/00_applications/portal/.env.example" "$PORTAL_ENV_FILE"
    check "Environment file" "pass" "copied from template"
    if grep -q -E "your-|TODO|CHANGEME" "$PORTAL_ENV_FILE" 2>/dev/null; then
      check "Environment secrets" "warn" "contains placeholder values — please configure them in 00_applications/portal/.env"
    fi
...
if [ "$QUICK_MODE" = "true" ]; then
  ...
  if curl -fs "$SUPABASE_REST_URL" > /dev/null 2>&1; then
    ...
    cd "$REPO_ROOT/01_platform_packages/database"
    mkdir -p "$REPO_ROOT/01_platform_packages/supabase/supabase/migrations"
    cp -r migrations/* "$REPO_ROOT/01_platform_packages/supabase/supabase/migrations/" 2>/dev/null || true
...
if [ "$QUICK_MODE" != "true" ]; then
  ...
  if [ "$START_TOOLS" = "true" ]; then
    if [ -f "$REPO_ROOT/10_infrastructure_as_code/docker/compose.tools.yml" ]; then
      echo -e "  ${INFO} Starting Docker Tools..."
      $COMPOSE_CMD -f "$REPO_ROOT/10_infrastructure_as_code/docker/compose.tools.yml" up -d > /dev/null 2>&1
  ...
  # Redis — isolated offload stack (12_distributed_cache_runtime/) or existing native listener
  REDIS_REQUIRED=true
  if [ "$REDIS_REQUIRED" = "true" ]; then
    if redis_reachable "$REDIS_URL"; then
      ...
      echo -e "  ${INFO} Starting Redis offload stack (12_distributed_cache_runtime/)..."
      ...
      check "Redis" "fail" "12_distributed_cache_runtime/docker-compose.yml missing"
      exit 1
    fi
  fi
  ...
  #   elif grep -q "open-webui:" "$REPO_ROOT/10_infrastructure_as_code/docker/compose.tools.yml" 2>/dev/null; then
  #     echo -e "  ${INFO} Starting Open WebUI (Docker Tools)..."
  #     $COMPOSE_CMD -f "$REPO_ROOT/10_infrastructure_as_code/docker/compose.tools.yml" up -d open-webui > /dev/null 2>&1
  #     for i in $(seq 1 15); do
...
if [ "${SKIP_RESTART:-false}" = "true" ]; then
  ...
  cd "$REPO_ROOT/00_applications/portal"
...
if [ "$START_CMS" = "true" ]; then
  start_extra_app \
    "cms" "$REPO_ROOT/00_applications/cms" "3001" \
    "$REPO_ROOT/run/cms.log" "$REPO_ROOT/run/.cms.pid" "CMS"
fi

if [ "$START_OVERVIEW" = "true" ]; then
  start_extra_app \
    "overview" "$REPO_ROOT/00_applications/overview" "3002" \
...
if [ "$RUN_E2E" = "true" ]; then
  ...
  bash "$REPO_ROOT/03_operations_automation/seed-e2e.sh"
  echo -e "  ${INFO} Running Playwright E2E tests..."
  cd "$REPO_ROOT"
  if [ -f "09_end_to_end_verification/.auth/user.json" ]; then
    rm -f "09_end_to_end_verification/.auth/user.json"
