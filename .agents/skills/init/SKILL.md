...
4. **Remind of Critical Standards**:
    ...
    - **Class Merging**: Always use [cn()](file:///home/tim/Arch/packages/ui/src/lib/utils.ts#L4) from `@repo/ui/lib/utils` (do not use raw template strings).
    ...
    - **Database Migrations**: Add zero-padded SQL files to [packages/database/migrations/](file:///home/tim/Arch/packages/database/migrations/) (e.g. `062_add_table.sql`) and commit them along with the regenerated types in [packages/supabase/src/database.types.ts](file:///home/tim/Arch/packages/supabase/src/database.types.ts).
    - **Policy Compiler**: Run `pnpm policy:gen` after changes to [tools/policy-compiler.cjs](file:///home/tim/Arch/tools/policy-compiler.cjs).
    - **Jest Testing**: When importing `@repo/*` packages in Next.js portal unit tests, update `moduleNameMapper` in [apps/portal/jest.config.js](file:///home/tim/Arch/apps/portal/jest.config.js).
