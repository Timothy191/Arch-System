...
/**
 * @fileoverview Checks that packages/database/migrations and
 * packages/supabase/migrations are in sync.
...
function checkSync() {
  ...
  if (onlyInDb.length > 0) {
    console.error("\n❌ Files in packages/database/migrations but missing from packages/supabase/migrations:");
    onlyInDb.forEach((f) => console.error(`  - ${f}`));
    hasErrors = true;
  }

  if (onlyInSupabase.length > 0) {
    console.error("\n❌ Files in packages/supabase/migrations but missing from packages/database/migrations:");
