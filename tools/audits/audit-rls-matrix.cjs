...
const RE_POLICY =
  /CREATE\s+POLICY\s+("?[\w]+"?)\s+ON\s+(?:[\w]+\.)?([\w]+)\s*(?:AS\s+[\w]+\s*)?(?:FOR\s+(SELECT|INSERT|UPDATE|DELETE|ALL))?([\s\S]*?);/gi;
...
migrationFiles.forEach((file) => {
  ...
  while ((match = RE_CREATE_TABLE.exec(sql)) !== null) {
    ...
    if (
      PARTITION_HINT.test(snippet) ||
      name.includes("_partition_") ||
      name.match(/_\d{4}_\d{2}$/)
    ) {
      continue;
    }
    if (!tables.has(name)) {
