...
cssFiles.forEach((file) => {
  ...
  if (matchesAll) {
    console.warn(
      `⚠️ [CSS Budget Warning] ${file}: Contains ${matchesAll.length} instance(s) of "transition: all". Use explicit properties (transform, opacity).`
    );
  ...
  if (matchesUniversal) {
    console.warn(
      `⚠️ [CSS Budget Warning] ${file}: Contains universal selector block "* {}". Ensure scope is restricted.`
    );
...
if (totalSize > BUDGET_KB * 1024) {
  console.error(
    `❌ [CSS Budget Exceeded] Bundle size ${totalKB} KB exceeds maximum budget of ${BUDGET_KB} KB.`
  );
  process.exit(1);
}
