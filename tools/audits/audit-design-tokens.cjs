...
if (fs.existsSync(layoutPath)) {
  ...
  const hasMono =
    layoutContent.includes("JetBrains_Mono({") || layoutContent.includes("Geist_Mono({");
...
files.forEach((relPath) => {
  ...
  lines.forEach((lineText, idx) => {
    ...
    // Rule A: Zero 'dark:' Mode Classes (Advisory Warning for Strict Light-Only Theme)
    if (lineText.includes("dark:") && !relPath.includes("tailwind.config")) {
      warnings.push({
        file: relPath,
        line: lineNum,
        type: "DARK_MODE_WARNING",
        description: `Use of 'dark:' responsive class is discouraged. Theme is strictly light-only. Found: "${trimmed}".`,
      });
    }
    ...
    if (isCSS && !isThemeOrCoreCss) {
      if (
        ...
        !lineText.includes("var(--shadow-") &&
        !lineText.includes("var(--glass-")
      ) {
        violations.push({
          file: relPath,
          line: lineNum,
