 ...
 * Usage: node 08_developer_tooling/design-audit.cjs
 */
/**
 * Static Design System Compliance Auditor
 * Checks the codebase (00_applications/portal, 01_platform_packages/ui, 01_platform_packages/theme) for visual style compliance.
 *
 * Checks:
...
const TARGET_DIRS = [
  path.join(ROOT, "00_applications", "portal"),
  path.join(ROOT, "01_platform_packages", "ui"),
  path.join(ROOT, "01_platform_packages", "theme"),
];
...
let ALLOWED_SHADOWS;
try {
  const tokensPath = path.join(ROOT, "01_platform_packages", "theme", "tokens.json");
...
function auditFile(filePath) {
  ...
  if (relPath.includes("01_platform_packages/theme") || relPath === "01_platform_packages/ui/src/globals.css") {
    return;
  }
  filesScanned++;
...
function generateReport() {
  ...
  lines.push("");
  lines.push(`Generated by \`08_developer_tooling/design-audit.cjs\` on ${new Date().toISOString()}.`);
  lines.push("");
  lines.push("## Summary");
  lines.push("");
