 ...
 * Checks the codebase (apps/portal, pkgs/ui, pkgs/theme) for visual style compliance.
 *
 * Checks:
...
const TARGET_DIRS = [
  ...
  path.join(ROOT, "pkgs", "ui"),
  path.join(ROOT, "pkgs", "theme"),
];
...
let ALLOWED_SHADOWS;
try {
  const tokensPath = path.join(ROOT, "pkgs", "theme", "tokens.json");
...
function auditFile(filePath) {
  ...
  if (relPath.includes("pkgs/theme") || relPath === "pkgs/ui/src/globals.css") {
    return;
  }
  filesScanned++;
