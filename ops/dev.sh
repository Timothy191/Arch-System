...
REDIS_COMPOSE="$REPO_ROOT/cache/docker-compose.yml"
...
if [ -f "$REPO_ROOT/ops/sync-assets-smart.cjs" ]; then
  node "$REPO_ROOT/ops/sync-assets-smart.cjs"
elif [ -f "$REPO_ROOT/ops/sync-assets.sh" ]; then
...
if [ "$QUICK_MODE" = "true" ]; then
  ...
  if curl -fs "$SUPABASE_REST_URL" > /dev/null 2>&1; then
    ...
    cd "$REPO_ROOT/pkgs/database"
    mkdir -p "$REPO_ROOT/pkgs/supabase/supabase/migrations"
    cp -r migrations/* "$REPO_ROOT/pkgs/supabase/supabase/migrations/" 2>/dev/null || true
...
if [ "$QUICK_MODE" != "true" ]; then
  ...
  # Redis — isolated offload stack (cache/) or existing native listener
  REDIS_REQUIRED=true
  if [ "$REDIS_REQUIRED" = "true" ]; then
    if redis_reachable "$REDIS_URL"; then
      ...
      echo -e "  ${INFO} Starting Redis offload stack (cache/)..."
      ...
      check "Redis" "fail" "cache/docker-compose.yml missing"
      exit 1
    fi
  fi
...
if [ "$RUN_E2E" = "true" ]; then
  ...
  bash "$REPO_ROOT/ops/seed-e2e.sh"
