...
cd pkgs/database && pnpx supabase status
```
...
```bash
./ops/backup-db.sh                                # Run a backup now
DATABASE_URL=postgresql://user:pass@host:5432/db ./ops/backup-db.sh  # Override connection
```

**Output in `ops/backups/`**:
...
0 3 * * * cd /path/to/arch-system && ./ops/backup-db.sh
```
...
./ops/restore-db.sh 2026-06-16_143022

# Non-interactive restore (CI/automation — use with extreme caution)
FORCE_RESTORE=true ./ops/restore-db.sh 2026-06-16_143022
```
