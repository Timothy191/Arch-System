...
ensureDir(join(ROOT, "toolchain/tools"));
...
// 2. Move config files from toolchain/ to toolchain/tools/
const configMoves = {
  "toolchain/tools/.fallowrc.json": "toolchain/tools/.fallowrc.json",
  "toolchain/tools/.gitleaks.toml": "toolchain/tools/.gitleaks.toml",
  "toolchain/tools/.lintstagedrc.mjs": "toolchain/tools/.lintstagedrc.mjs",
  "toolchain/tools/.markdownlint.json": "toolchain/tools/.markdownlint.json",
  "toolchain/tools/mcp.json": "toolchain/tools/mcp.json", // no leading dot as per user spec
  ".memory/toolchain/.memoryignore": ".memory/.memory/toolchain/.memoryignore",
  "toolchain/tools/.secretlintignore": "toolchain/tools/.secretlintignore",
  "toolchain/tools/.secretlintrc.json": "toolchain/tools/.secretlintrc.json",
  "toolchain/tools/.syncpackrc.js": "toolchain/tools/.syncpackrc.js",
  "toolchain/tools/commitlint.config.mjs": "toolchain/tools/commitlint.config.mjs",
  "toolchain/tools/eslint.boundaries.cjs": "toolchain/tools/eslint.boundaries.cjs",
  "toolchain/tools/knip.json": "toolchain/tools/knip.json",
  "toolchain/tools/lighthouserc.json": "toolchain/tools/lighthouserc.json",
};
...
const dockerMoves = {
  ...
  "infra/obs/docker-compose.yml": "infra/obs/docker-compose.yml",
};
...
// 4. Move infra/cache/ to infra/cache/
...
if (existsSync(pkgJsonPath)) {
  ...
  content = content.replace(
    ...
    '"config": "toolchain/tools/knip.json"',
  );
  content = content.replace(
    /"config":\s*"config\/\.markdownlint\.json"/g,
    '"config": "toolchain/tools/.markdownlint.json"',
  );
  content = content.replace(
    /"config":\s*"config\/\.lintstagedrc\.mjs"/g,
    '"config": "toolchain/tools/.lintstagedrc.mjs"',
  );
  content = content.replace(
    /"config":\s*"config\/\.syncpackrc\.js"/g,
    '"config": "toolchain/tools/.syncpackrc.js"',
  );
  content = content.replace(
    /"extends":\s*\[\s*"\.\/config\/commitlint\.config\.mjs"\s*\]/g,
    '"extends": ["./toolchain/tools/commitlint.config.mjs"]',
  );
  content = content.replace(
    /docker\/docker-compose\.monitoring\.yml/g,
    "infra/obs/docker-compose.yml",
  );
...
function referenceReplacer(content, filePath) {
  ...
  updated = updated.replace(/config\/\.fallowrc\.json/g, "toolchain/tools/.fallowrc.json");
  updated = updated.replace(/config\/\.gitleaks\.toml/g, "toolchain/tools/.gitleaks.toml");
  updated = updated.replace(/config\/\.lintstagedrc\.mjs/g, "toolchain/tools/.lintstagedrc.mjs");
  updated = updated.replace(/config\/\.markdownlint\.json/g, "toolchain/tools/.markdownlint.json");
  updated = updated.replace(/config\/\.mcp\.json/g, "toolchain/tools/mcp.json");
  updated = updated.replace(/config\/\.secretlintignore/g, "toolchain/tools/.secretlintignore");
  updated = updated.replace(/config\/\.secretlintrc\.json/g, "toolchain/tools/.secretlintrc.json");
  updated = updated.replace(/config\/\.syncpackrc\.js/g, "toolchain/tools/.syncpackrc.js");
  updated = updated.replace(
    /config\/commitlint\.config\.mjs/g,
    "toolchain/tools/commitlint.config.mjs",
  );
  updated = updated.replace(
    /config\/eslint\.boundaries\.cjs/g,
    "toolchain/tools/eslint.boundaries.cjs",
  );
  updated = updated.replace(/config\/knip\.json/g, "toolchain/tools/knip.json");
  updated = updated.replace(/config\/lighthouserc\.json/g, "toolchain/tools/lighthouserc.json");
  updated = updated.replace(/config\/\.memoryignore/g, ".memory/.memory/toolchain/.memoryignore");
  ...
  updated = updated.replace(/redis-topology\//g, "infra/cache/");
...
for (const file of movedDockerFiles) {
  ...
  if (existsSync(filePath)) {
    ...
    content = content.replace(/\.\/config\/nginx\.conf/g, "../../toolchain/nginx.conf");
...
const monitoringCompose = join(ROOT, "infra/obs/docker-compose.yml");
