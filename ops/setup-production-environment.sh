...
# Usage: ./ops/setup-production-environment.sh [options]
#
...
show_rocky_linux_guidance() {
  if [ "$OS" = "rocky" ] || [ "$OS" = "rhel" ] || [ "$OS" = "centos" ]; then
    ...
    echo
    echo -e "${BOLD}See: ${CYAN}ops/ROCKY_LINUX_COMPATIBILITY.md${NC} for detailed guide"
    echo
...
setup_monitoring() {
  ...
  local monitoring_file="$REPO_ROOT/infra/obs/docker-compose.yml"
...
print_summary() {
  ...
  if [ "$OS" = "rocky" ] || [ "$OS" = "rhel" ] || [ "$OS" = "centos" ]; then
    ...
    echo -e "  • Compatibility guide: ${CYAN}ops/ROCKY_LINUX_COMPATIBILITY.md${NC}"
