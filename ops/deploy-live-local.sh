...
python3 "$REPO_ROOT/ops/ensure_reachability.py" "$selected_ip" "$anon_key" "$service_key"
...
if [ -f "$REPO_ROOT/infra/obs/docker-compose.yml" ]; then
  $COMPOSE_CMD -f "$REPO_ROOT/infra/obs/docker-compose.yml" up -d
fi
