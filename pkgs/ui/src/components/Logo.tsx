...
interface LogoProps {
  ...
  /** Login wordmark — left half reads lighter than the right */
  splitTone?: boolean;
}

export function Logo({ className, variant = "default", splitTone = false }: LogoProps) {
  const gradientId = useId();
  const fill = splitTone ? `url(#${gradientId})` : "currentColor";
  ...
  return (
    <svg
      ...
      className={cn(!splitTone && "fill-current text-arch-accent-blue", className)}
      data-split-tone={splitTone ? "" : undefined}
      aria-hidden="true"
    >
      {splitTone ? (
        <defs>
          ...
          >
            <stop offset="0%" stopColor="oklch(52% 0.014 265)" />
            <stop offset="46%" stopColor="oklch(40% 0.013 265)" />
            <stop offset="54%" stopColor="oklch(26% 0.012 265)" />
            <stop offset="100%" stopColor="oklch(12% 0.01 265)" />
          </linearGradient>
        </defs>
      ) : null}
      <path
