...
All policy enforcement is consolidated into `08_developer_tooling/policy-compiler.cjs` and `08_developer_tooling/design-audit.cjs`.

## 1. Adding Architectural Boundaries

To add a new dependency rule (e.g., "UI packages cannot depend on API packages"):

1. Open `08_developer_tooling/apply-project-tags.cjs` to ensure the relevant project directories get the correct `scope:*` tags.
...
1. Open `08_developer_tooling/design-audit.cjs`.
...
1. Open `08_developer_tooling/audit-rls.cjs`.
2. The script parses SQL migrations in `01_platform_packages/database/migrations`. Add new parsers to verify that specific policies like `CREATE POLICY` are present for the new table.
