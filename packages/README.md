...
| `@repo/agents` | Agent orchestration and workflow execution components | apps/portal, CI/CD | Manages AI agent lifecycles, memory stores, and LLM reasoning pipelines. |
| ------------------------- | -------------------------------------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `@repo/database` | SQL migrations (source of truth) | apps/portal (via Supabase) | Migration files are the canonical schema definition. Never edit `packages/supabase/supabase/migrations/` directly. |
...
| `@repo/eslint-config` | Shared ESLint configurations | All packages/apps | Provides `library.js`, `next.js`, and `react-internal.js` configurations. Tooling package. |
| `@repo/logger` | Centralized logging utilities and formatting | apps/portal, other packages | Structured logging system with Winston/Pino integration and context propagation for distributed tracing. |
...
| `@repo/typescript-config` | Shared TypeScript configuration | All packages/apps | Tooling package. Ensures consistent TS settings across the monorepo. |
...
```
@repo/eslint-config → All packages/apps
@repo/typescript-config → All packages/apps
```
...
3. Add the package to `pnpm-workspace.yaml` if not using the `packages/*` pattern
...
1. Edit `packages/theme/tokens.json`
2. Run `pnpm --filter @repo/theme build`
3. Commit both `tokens.json` and `packages/theme/src/tokens/generated.ts`

### Database Types (@repo/database → @repo/supabase)

1. Add migration to `packages/database/migrations/NNN_description.sql`
2. Run `pnpm --filter @repo/database supabase:push`
3. Run `pnpm --filter @repo/database supabase:gen`
4. Commit migration + updated `packages/supabase/src/database.types.ts`
