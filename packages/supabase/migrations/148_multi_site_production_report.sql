...
DROP POLICY IF EXISTS "Authenticated users can read excavator haul logs" ON excavator_haul_logs;
DROP POLICY IF EXISTS "Authenticated users can insert/update excavator haul logs" ON excavator_haul_logs;
DROP POLICY IF EXISTS "Authenticated users can read excavator truck tallies" ON excavator_truck_tallies;
DROP POLICY IF EXISTS "Authenticated users can insert/update excavator truck tallies" ON excavator_truck_tallies;
DROP POLICY IF EXISTS "Authenticated users can read dozer rollover logs" ON dozer_rollover_logs;
DROP POLICY IF EXISTS "Authenticated users can insert/update dozer rollover logs" ON dozer_rollover_logs;
DROP POLICY IF EXISTS "Authenticated users can read ancillary shift logs" ON ancillary_shift_logs;
DROP POLICY IF EXISTS "Authenticated users can insert/update ancillary shift logs" ON ancillary_shift_logs;

DO $$ BEGIN
  CREATE POLICY "Authenticated users can read excavator haul logs"
    ON excavator_haul_logs FOR SELECT TO authenticated USING (EXISTS (
      SELECT 1 FROM employees e
      WHERE e.auth_id = auth.uid()
        AND (
          e.role = 'admin'
          OR e.department_id = excavator_haul_logs.department_id
          OR excavator_haul_logs.department_id = ANY(e.accessible_departments)
        )
    ));
  CREATE POLICY "Authenticated users can insert/update excavator haul logs"
    ON excavator_haul_logs FOR ALL TO authenticated
    USING (EXISTS (
      SELECT 1 FROM employees e
      WHERE e.auth_id = auth.uid()
        AND (
...
DO $$ BEGIN
  CREATE POLICY "Authenticated users can read excavator truck tallies"
    ON excavator_truck_tallies FOR SELECT TO authenticated
    USING (EXISTS (
      SELECT 1
      FROM excavator_haul_logs ehl
      JOIN employees e ON e.auth_id = auth.uid()
      WHERE ehl.id = excavator_truck_tallies.haul_log_id
        AND (
          e.role = 'admin'
          OR e.department_id = ehl.department_id
          OR ehl.department_id = ANY(e.accessible_departments)
        )
    ));
  CREATE POLICY "Authenticated users can insert/update excavator truck tallies"
    ON excavator_truck_tallies FOR ALL TO authenticated
    USING (EXISTS (
      SELECT 1
      FROM excavator_haul_logs ehl
      JOIN employees e ON e.auth_id = auth.uid()
...
DO $$ BEGIN
  CREATE POLICY "Authenticated users can read dozer rollover logs"
    ON dozer_rollover_logs FOR SELECT TO authenticated USING (EXISTS (
      SELECT 1 FROM employees e
      WHERE e.auth_id = auth.uid()
        AND (
          e.role = 'admin'
          OR e.department_id = dozer_rollover_logs.department_id
          OR dozer_rollover_logs.department_id = ANY(e.accessible_departments)
        )
    ));
  CREATE POLICY "Authenticated users can insert/update dozer rollover logs"
    ON dozer_rollover_logs FOR ALL TO authenticated
    USING (EXISTS (
      SELECT 1 FROM employees e
      WHERE e.auth_id = auth.uid()
        AND (
...
DO $$ BEGIN
  CREATE POLICY "Authenticated users can read ancillary shift logs"
    ON ancillary_shift_logs FOR SELECT TO authenticated USING (EXISTS (
      SELECT 1 FROM employees e
      WHERE e.auth_id = auth.uid()
        AND (
          e.role = 'admin'
          OR e.department_id = ancillary_shift_logs.department_id
          OR ancillary_shift_logs.department_id = ANY(e.accessible_departments)
        )
    ));
  CREATE POLICY "Authenticated users can insert/update ancillary shift logs"
    ON ancillary_shift_logs FOR ALL TO authenticated
    USING (EXISTS (
      SELECT 1 FROM employees e
      WHERE e.auth_id = auth.uid()
        AND (
