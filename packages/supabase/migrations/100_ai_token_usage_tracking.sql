...
CREATE INDEX idx_ai_token_usage_date_range ON ai_token_usage(((created_at AT TIME ZONE 'UTC')::date));
...
SELECT 
  ...
  SUM(total_tokens) as total_tokens,
  SUM(total_cost_usd_cents) as total_cost_cents,
  DATE_TRUNC('week', ai_token_usage.created_at) as week_start
FROM ai_token_usage
LEFT JOIN departments d ON d.id = department_id
GROUP BY department_id, d.name, model_name, DATE_TRUNC('week', ai_token_usage.created_at)
