...
const SUPABASE_SERVICE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1yd2h0eGJocnp5dHRsc3l1b2ZjIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4Njc3NzA4MywiZXhwIjoyMTAyMzUzMDgzfQ.iPXzOvs8RanPA5hI33f8x2_ThgR7h72E41vRHDxkOh8";
...
async function main() {
  ...
  // 2. Create Machines
  const machines = [
    { name: "DRILL-01", machine_type: "Drill", serial_number: "DR-001", bin_factor: 1.2, active: true, department_id: deptId },
    { name: "DRILL-02", machine_type: "Drill", serial_number: "DR-002", bin_factor: 1.2, active: true, department_id: deptId },
    { name: "EXC-01", machine_type: "Excavator", serial_number: "EX-001", bin_factor: 2.5, active: true, department_id: deptId },
  ];
  
  let { data: existingMachines } = await supabase.from("machines").select("*").eq("department_id", deptId);
  if (!existingMachines || existingMachines.length === 0) {
    const { data: newMachines, error: machineErr } = await supabase.from("machines").insert(machines).select();
  ...
  const { data: insertedOperators, error: opErr } = await supabase.from("operators").upsert(operators, { onConflict: "employee_code" }).select();
  if (opErr) console.error("Operator error:", opErr);
  
  // 4. Create a Site
  const { data: sites, error: siteErr } = await supabase.from("sites").upsert([
    { name: "North Pit", site_code: "NP-01", active: true }
  ], { onConflict: "site_code" }).select();
  ...
  for (let i = 0; i < 20; i++) {
    ...
    let { data: dailyLog, error: dailyLogErr } = await supabase.from("daily_logs").upsert({
        department_id: deptId,
        log_date: dateStr,
        shift: "day",
    }, { onConflict: "department_id, log_date, shift" }).select().single();
  ...
  let { data: todayLog, error: todayLogErr } = await supabase.from("daily_logs").upsert({
      department_id: deptId,
      log_date: todayStr,
      shift: "day",
  }, { onConflict: "department_id, log_date, shift" }).select().single();
