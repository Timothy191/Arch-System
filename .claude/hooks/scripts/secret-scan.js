...
const PATTERNS = [
  ...
  {
    name: "AWS Secret Key",
    re: /\b(?:aws_)?secret(?:_access)?_key\s*[=:]\s*["']?[A-Za-z0-9/+=]{40}["']?/i,
  },
  ...
  {
    name: "OpenAI API Key",
    re: /\bsk-(?:proj-)?(?!ant-)[A-Za-z0-9_\-]{20,}\b/,
  },
  ...
  },
  {
    ...
    re: /\b(?:api[_\-]?key|api[_\-]?secret|secret|token)\s*[=:]\s*["'][A-Za-z0-9_\-]{20,}["']/i,
  },
];
...
(async () => {
  ...
  console.error(
    `[pro-workflow] secret-scan: detected ${hit.name} near line ${hit.line}: ${hit.snippet}... — remove or load from env.`,
  );
  process.exit(2);
})();
