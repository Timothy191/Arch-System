...
# Syncs global assets from the root shared/ directory to
...
echo "  → Synchronizing global assets from root shared/..."
...
for TARGET in "${TARGETS[@]}"; do
  ...
  cp -R "$REPO_ROOT/shared/." "$FULL_TARGET/"
  
  echo "  ✓ Successfully synced assets to $TARGET"
done

echo "  ✓ Asset synchronization complete!"
echo "  → Root shared/ at $REPO_ROOT/shared is now the single source of truth"
