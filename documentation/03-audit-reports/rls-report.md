# RLS Policy Audit

Generated by `tools/audits/audit-rls.cjs` from `packages/database/migrations`.

## Summary

| Metric                                    | Count |
| ----------------------------------------- | ----- |
| Migrations scanned                        | 116   |
| Tables declared                           | 88    |
| Tables with RLS enabled                   | 88    |
| Tables missing RLS (CRITICAL)             | 0     |
| Tables with suspicious policies (WARNING) | 0     |
| Total CREATE POLICY statements            | 367   |

## CRITICAL — Tables Missing Row Level Security

_None. Every CREATE TABLE has a matching ALTER TABLE … ENABLE ROW LEVEL SECURITY somewhere in the migration sequence._

## WARNING — Suspicious Policies

_None._

## Tables With RLS Enabled

| Table                                  | RLS enabled in                                                                                  |
| -------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `ab_test_results`                      | 093_feature_flags.sql                                                                           |
| `access_logs`                          | 028_access_control_system.sql                                                                   |
| `access_logs_archive`                  | 033_access_logs_weekly_archival.sql                                                             |
| `ai_token_usage`                       | 102_ai_token_usage_tracking.sql                                                                 |
| `ai_usage_logs`                        | 032_ai_usage_logs.sql                                                                           |
| `ancillary_shift_logs`                 | 150_multi_site_production_report.sql                                                            |
| `audit_logs`                           | 007_audit_logs.sql                                                                              |
| `badges`                               | 028_access_control_system.sql                                                                   |
| `breakdowns`                           | 004_breakdowns.sql                                                                              |
| `cache_anomalies`                      | 067_cache_events.sql                                                                            |
| `cache_events`                         | 067_cache_events.sql                                                                            |
| `card_printers`                        | 076_card_printing_infrastructure.sql                                                            |
| `card_templates`                       | 076_card_printing_infrastructure.sql                                                            |
| `compliance_audit_runs`                | 153_operational_compliance_checks.sql                                                           |
| `control_room_outbox`                  | 162_idempotency_and_outbox.sql                                                                  |
| `control_room_shift_reports`           | 097_control_room_shift_reports.sql                                                              |
| `daily_logs`                           | 001_initial.sql, 020_partition_time_series.sql                                                  |
| `data_integrity_issues`                | 088_data_integrity_issues.sql                                                                   |
| `delay_categories`                     | 002_control_room_tables.sql, 068_delay_entries_table.sql, standalone_unblock_missing_tables.sql |
| `delay_entries`                        | 068_delay_entries_table.sql, standalone_unblock_missing_tables.sql                              |
| `delay_entries_archive`                | 068_delay_entries_table.sql                                                                     |
| `departments`                          | 001_initial.sql                                                                                 |
| `document_versions`                    | 036_documents.sql                                                                               |
| `documents`                            | 036_documents.sql                                                                               |
| `dozer_rollover_logs`                  | 150_multi_site_production_report.sql                                                            |
| `dozer_rolls`                          | 002_control_room_tables.sql                                                                     |
| `dozer_rolls_archive`                  | 046_control_room_archiving.sql                                                                  |
| `drill_operations`                     | 024_drill_operations.sql                                                                        |
| `drill_operations_archive`             | 027_drill_shifts_and_archiving.sql                                                              |
| `embedding_cache`                      | 059_embedding_cache.sql                                                                         |
| `employees`                            | 001_initial.sql                                                                                 |
| `engineering_notes`                    | 003_control_room_revisions.sql                                                                  |
| `engineering_notes_archive`            | 046_control_room_archiving.sql                                                                  |
| `equipment`                            | 035_fleet_and_equipment_tables.sql                                                              |
| `excavator_activity`                   | 002_control_room_tables.sql                                                                     |
| `excavator_activity_archive`           | 046_control_room_archiving.sql                                                                  |
| `excavator_dumper_assignments`         | 008_excavator_activity_redesign.sql                                                             |
| `excavator_dumper_assignments_archive` | 046_control_room_archiving.sql                                                                  |
| `excavator_haul_logs`                  | 150_multi_site_production_report.sql                                                            |
| `excavator_truck_tallies`              | 150_multi_site_production_report.sql                                                            |
| `feature_flag_exposures`               | 093_feature_flags.sql                                                                           |
| `feature_flags`                        | 093_feature_flags.sql                                                                           |
| `fleet`                                | 035_fleet_and_equipment_tables.sql                                                              |
| `fuel_logs`                            | 001_initial.sql                                                                                 |
| `generated_reports`                    | 002_control_room_tables.sql                                                                     |
| `hourly_loads`                         | 002_control_room_tables.sql, 003_control_room_revisions.sql, 020_partition_time_series.sql      |
| `idempotency_keys`                     | 162_idempotency_and_outbox.sql                                                                  |
| `issued_cards`                         | 076_card_printing_infrastructure.sql                                                            |
| `machine_configurations`               | 042_machine_configurations.sql                                                                  |
| `machine_hours`                        | 001_initial.sql                                                                                 |
| `machine_operations`                   | 002_control_room_tables.sql                                                                     |
| `machine_operations_archive`           | 046_control_room_archiving.sql                                                                  |
| `machine_telemetry`                    | 025_machine_telemetry.sql                                                                       |
| `machine_telemetry_archive`            | 025_machine_telemetry.sql                                                                       |
| `machines`                             | 001_initial.sql                                                                                 |
| `material_density`                     | 073_production_summary_view.sql, 094_enable_rls_material_density.sql                            |
| `materialized_view_refresh_log`        | 065_materialized_view_refresh_optimization.sql                                                  |
| `memory_embeddings`                    | 009_ai_memory.sql                                                                               |
| `mine_blocks`                          | 008_excavator_activity_redesign.sql                                                             |
| `operational_delays`                   | 003_control_room_revisions.sql                                                                  |
| `operational_delays_archive`           | 046_control_room_archiving.sql                                                                  |
| `operators`                            | 002_control_room_tables.sql                                                                     |
| `personnel`                            | 028_access_control_system.sql                                                                   |
| `print_jobs`                           | 076_card_printing_infrastructure.sql                                                            |
| `production_logs`                      | 001_initial.sql, 072_partition_production_logs.sql                                              |
| `quick_feedback`                       | 015_user_feedback.sql                                                                           |
| `report_templates`                     | 002_control_room_tables.sql                                                                     |
| `roles`                                | 070_control_room_operator_role_and_lookup.sql                                                   |
| `safety_incident_categories`           | 006_safety_department.sql                                                                       |
| `safety_incidents`                     | 006_safety_department.sql                                                                       |
| `safety_severities`                    | 006_safety_department.sql                                                                       |
| `satellite_deformations`               | 078_satellite_insar_deformations.sql                                                            |
| `secrets_rotation_log`                 | 086_secrets_rotation_log.sql                                                                    |
| `shift_completeness_alerts`            | 087_shift_completeness_alerts.sql                                                               |
| `shift_integrity_reports`              | 089_shift_integrity_reports.sql                                                                 |
| `shift_notes`                          | 002_control_room_tables.sql                                                                     |
| `shift_status`                         | 0145_shift_closeout.sql, standalone_unblock_missing_tables.sql                                  |
| `sites`                                | 002_control_room_tables.sql                                                                     |
| `slo_metrics`                          | 092_slo_monitoring.sql                                                                          |
| `sync_watermarks`                      | 031_embedding_sync_watermarks.sql                                                               |
| `tire_inspections`                     | 0146_tire_management.sql                                                                        |
| `tires`                                | 0146_tire_management.sql                                                                        |
| `user_feedback`                        | 015_user_feedback.sql                                                                           |
| `vector_search_cache`                  | 064_vector_search_query_optimization.sql                                                        |
| `vector_search_performance`            | 064_vector_search_query_optimization.sql                                                        |
| `visitors`                             | 028_access_control_system.sql                                                                   |
| `webhook_delivery_logs`                | 017_webhooks.sql                                                                                |
| `webhook_endpoints`                    | 017_webhooks.sql                                                                                |
