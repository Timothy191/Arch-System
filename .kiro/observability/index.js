...
function getSessionId() {
  return (
    (process.env.CLAUDE_SESSION_ID || "default").replace(
      /[^a-zA-Z0-9_-]/g,
      "",
    ) || "default"
  );
...
function getStats() {
  try {
    ...
    const metricRaw = fs.existsSync(METRICS_LOG)
      ? fs.readFileSync(METRICS_LOG, "utf8").trim()
      : "";
  ...
  try {
    ...
    for (let i = lines.length - 1; i >= 0 && results.length < limit; i--) {
      try {
        ...
        if (
          query.name &&
          !t.tool?.includes(query.name) &&
          !t.name?.includes(query.name)
        )
          continue;
        results.push(t);
      } catch {}
