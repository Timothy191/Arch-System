...
function getGitDiff(filePath) {
  ...
  try {
    const staged = execSync(`git diff --cached -- "${relPath}"`, {
      cwd: PROJ,
      encoding: "utf8",
      maxBuffer: 102400,
    });
    const working = execSync(`git diff -- "${relPath}"`, {
      cwd: PROJ,
      encoding: "utf8",
      maxBuffer: 102400,
    });
    return {
      staged,
      working,
      hasChanges: staged.length > 0 || working.length > 0,
    };
  } catch {
...
async function main() {
  ...
  if (!filePath && !newContent) {
    console.error(
      "[file-diff] Usage: pipe JSON with file_path, new_string, old_string",
    );
    process.exit(1);
  }
