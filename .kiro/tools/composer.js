...
function executeStep(step) {
  ...
  switch (action) {
    case "read": {
      ...
      if (!filePath)
        return { step: "read", error: "No file_path", status: "error" };
    ...
    case "edit": {
      ...
      if (!filePath || oldStr === undefined)
        return {
          step: "edit",
          error: "Missing file_path or old_string",
          status: "error",
        };
      const absPath = path.isAbsolute(filePath)
        ? filePath
      ...
      if (!content.includes(oldStr))
        return { step: "edit", error: "old_string not found", status: "error" };
    ...
    case "write": {
      ...
      if (!filePath)
        return { step: "write", error: "No file_path", status: "error" };
      const absPath = path.isAbsolute(filePath)
        ? filePath
        : path.join(PROJ, filePath);
    ...
    case "bash": {
      ...
      try {
        ...
        return {
          ...
          status: "ok",
          output: { stdout: result.slice(0, 5000), exitCode: 0 },
        };
      } catch (err) {
        return {
          step: "bash",
    ...
    case "lint": {
      const filePath = params.file_path || params.filePath;
      if (!filePath)
        return { step: "lint", error: "No file_path", status: "error" };
      const relPath = path.relative(PROJ, filePath);
      try {
        const result = execSync(
          `pnpm exec eslint "${relPath}" --max-warnings 0`,
          { cwd: PROJ, encoding: "utf8", maxBuffer: 102400, timeout: 15000 },
        );
        return {
          step: "lint",
          filePath,
          status: "ok",
          output: { passed: true, message: "Lint passed" },
        };
      } catch (err) {
        return {
          step: "lint",
    ...
    case "typecheck": {
      try {
        ...
        return {
          step: "typecheck",
          status: "ok",
          output: { passed: true, message: "Typecheck passed" },
        };
      } catch (err) {
        return {
          step: "typecheck",
...
async function main() {
  ...
  if (steps.length === 0) {
    console.log(
      JSON.stringify({
        error: "No steps provided. Use { steps: [...] } or a single step.",
        usage: "See .kiro/tools/composer.js",
      }),
    );
    process.exit(1);
  }
  ...
  if (input.combineReads) {
    const readResults = results.filter(
      (r) => r.step === "read" && r.status === "ok",
    );
    if (readResults.length > 0) {
      output.combined = readResults.map((r) => ({
        file: r.filePath,
