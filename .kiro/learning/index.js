...
function generateSkills(patterns, goals) {
  ...
  if (frequentSequences.length > 0) {
    ...
    skills.push({
      ...
      suggestion:
        "Consider creating a workflow template for this common sequence",
    });
  }
  ...
  for (const [goal, count] of commonGoals) {
    skills.push({
      ...
      suggestion:
        "Consider creating a specialized skill for this recurring task",
    });
  }

  existing.skills.push(...skills);
  if (existing.skills.length > 100)
    existing.skills.splice(0, existing.skills.length - 100);
...
async function main() {
  ...
  switch (cmd) {
        ...
        JSON.stringify(
          {
            analyzed: {
              ...
              audit_entries: auditLines.length,
            },
            patterns: {
              frequent_sequences: Object.entries(patterns.toolSequences)
                ...
                .slice(0, 10),
              error_rate:
                Object.keys(patterns.errorPatterns).length > 0
                  ? Object.values(patterns.errorPatterns).reduce(
                      (a, b) => a + b,
                      0,
    ...
    case "recall": {
      ...
      if (!task) {
        console.log(
          JSON.stringify({ skills: [], message: "No task provided" }),
        );
        break;
      }
      ...
      console.log(
        JSON.stringify(
          { task, relevant_skills: relevant.length, skills: relevant },
          null,
          2,
        ),
      );
        ...
        JSON.stringify(
          {
            ...
            by_type: Object.fromEntries(
              Object.entries(byType).map(([t, names]) => [
                t,
                { count: names.length, examples: names.slice(0, 3) },
              ]),
            ),
          },
          null,
