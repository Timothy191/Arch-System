...
function loadState() {
  try {
    if (fs.existsSync(SWARM_FILE))
      return JSON.parse(fs.readFileSync(SWARM_FILE, "utf8"));
  } catch {}
...
class Foreman {
  ...
  async _executePipeline() {
    ...
    while (pending.length > 0 || running.length > 0) {
      ...
      if (idx >= 0) {
        const taskInRun = this.state.runs[idx].tasks.find(
          (t) => t.id === worker.taskId,
        );
        if (taskInRun) {
    ...
    if (runIdx >= 0) {
      this.state.runs[runIdx].status =
        failed.length === 0 ? "completed" : "completed_with_errors";
  ...
  async _spawnWorker(task) {
    ...
    worker.process.stdout.on("data", (d) => {
      stdout += d.toString();
    });
    worker.process.stderr.on("data", (d) => {
      stderr += d.toString();
    });
    ...
    worker._done = new Promise((resolve) => {
      ...
      worker.process.on("close", (code) => {
        ...
        if (code === 0) {
          try {
            worker.result = JSON.parse(stdout);
          } catch {
...
async function main() {
  ...
  switch (cmd) {
    ...
    case "decompose": {
      const input = parseInput(raw);
      const tasks = decomposeGoal(
        input.goal || input.description || "",
        input.context || {},
      );
