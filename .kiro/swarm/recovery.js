  ...
  const strategies = [
    {
      ...
      description: "Increase worker timeout",
    },
    {
      match: /memory/i,
      action: "reduce_load",
    ...
    },
    {
      ...
      description: "Verify prerequisites exist",
    },
    {
      match: /permission/i,
      action: "escalate_permissions",
      description: "Escalate tool permissions",
    },
  ];
...
async function main() {
  ...
  switch (cmd) {
    ...
    case "status": {
      ...
      const statuses = Object.entries(circuit.circuits || {}).map(
        ([name, c]) => ({
          name,
          state: c.state,
          failures: c.failures,
    ...
    case "reset": {
      ...
      if (name && circuit.circuits[name]) {
        ...
        saveCircuit(circuit);
        console.log(
          JSON.stringify({ status: "ok", circuit: name, action: "reset" }),
        );
      } else if (!name) {
        circuit.circuits = {};
        saveCircuit(circuit);
    ...
    case "recover": {
      const input = parseInput(raw);
      const recovered = recoverWorker(
        input.task || {},
        input.reason || "unknown",
      );
      console.log(
        JSON.stringify(
          { recovered, recovery_count: recovered?.recovery_count || 0 },
          null,
          2,
        ),
      );
