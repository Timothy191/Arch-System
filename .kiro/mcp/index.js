...
function loadMcpConfig() {
  try {
    return JSON.parse(fs.readFileSync(MCP_CONFIG, "utf8")).mcpServers || {};
  } catch {
    return {};
  }
}
...
async function cmdDiscover() {
  ...
  if (n8nConfig) {
    ...
    const n8nTools = [
      ...
      "n8n_search_workflows_by_tag",
    ];
    for (const t of n8nTools) {
      registry.tools[t] = {
        server: "n8n",
        description: `n8n workflow tool: ${t}`,
      };
    }
  }
  ...
  console.log(
    JSON.stringify({
      ...
      tools: toolCount,
      errors: Object.entries(registry.servers).filter(
        ([, s]) => s.status === "error",
      ).length,
    }),
  );
  ...
  console.log(
    JSON.stringify({
      ...
      by_server: Object.fromEntries(
        Object.entries(byServer).map(([s, tools]) => [
          s,
    ...
    default:
      console.log(
        JSON.stringify({
          ...
          usage: "discover|status|find <name>",
        }),
      );
  }
}
