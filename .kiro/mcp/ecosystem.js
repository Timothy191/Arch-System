...
const BUILTIN_SERVERS = {
  ...
  "mcp-database": {
    ...
    args: [
      ...
      path.join(PROJ, "ltm", "store", "traces.db"),
    ],
...
function loadRegistry() {
  try {
    ...
    return { version: 2, servers: {}, tools: {}, updated_at: null };
  }
}
...
function loadRuntime() {
  try {
    return JSON.parse(fs.readFileSync(MCP_RUNTIME, "utf8"));
  } catch {
    return { processes: {} };
  }
}
...
function listServers() {
  ...
  return Object.entries(mcpConfig.mcpServers || {}).map(([name, config]) => ({
    ...
    description:
      registry.servers[name]?.description || config.description || "",
...
async function startServer(name) {
  ...
  const runtime = loadRuntime();
  if (runtime.processes[name])
    return { status: "ok", message: `Already running: ${name}` };
  ...
  switch (cmd) {
    ...
    case "add-builtin": {
      ...
      if (!name || !BUILTIN_SERVERS[name]) {
        console.error(
          `Available builtins: ${Object.keys(BUILTIN_SERVERS).join(", ")}`,
        );
        process.exit(1);
      }
      ...
      console.log(
        JSON.stringify({ ...result, description: config.description }),
      );
      break;
    }
    ...
    case "remove": {
      const name = process.argv[3];
      if (!name) {
        console.error("Usage: mcp-ecosystem remove <name>");
        process.exit(1);
      }
    ...
    case "start": {
      const name = process.argv[3];
      if (!name) {
        console.error("Usage: mcp-ecosystem start <name>");
        process.exit(1);
      }
    ...
    case "stop": {
      const name = process.argv[3];
      if (!name) {
        console.error("Usage: mcp-ecosystem stop <name>");
        process.exit(1);
      }
    ...
    case "discover": {
      ...
      console.log(
        JSON.stringify(
          { discovered: results.length, servers: results },
          null,
          2,
        ),
      );
    ...
    case "tool": {
      ...
      if (!toolName) {
        console.error("Usage: mcp-ecosystem tool <name>");
        process.exit(1);
      }
    ...
    default:
      console.log(
        "Commands: add | add-builtin | remove | list | start | stop | discover | tool | status",
      );
  }
}
