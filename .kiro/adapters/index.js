...
function loadConfig() {
  try {
    ...
    return { active_provider: "jsonl", providers: {} };
  }
}
...
async function queryQdrant(text, topK, cfg) {
  try {
    ...
    console.error(
      `[adapter] Qdrant error: ${err.message}. Falling back to jsonl.`,
    );
    return queryJsonl(text, topK);
  }
}
...
async function queryChroma(text, topK, cfg) {
  try {
    const res = await fetch(
      `${cfg.url}/api/v1/collections/${cfg.collection}/query`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    console.error(
      `[adapter] Chroma error: ${err.message}. Falling back to jsonl.`,
    );
    return queryJsonl(text, topK);
  }
}
...
async function ingestQdrant(items, cfg) {
  try {
    ...
    console.error(
      `[adapter] Qdrant ingest error: ${err.message}. Falling back to jsonl.`,
    );
    return ingestJsonl(items);
  }
}
...
async function ingestChroma(items, cfg) {
  try {
    ...
    const res = await fetch(
      `${cfg.url}/api/v1/collections/${cfg.collection}/add`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    console.error(
      `[adapter] Chroma ingest error: ${err.message}. Falling back to jsonl.`,
    );
    return ingestJsonl(items);
  }
}
...
async function embed(text) {
  ...
  if (result.status === 0) {
    try {
      return JSON.parse(result.stdout.trim()).vector;
    } catch {}
  }
...
async function status() {
  ...
  for (const [name, cfg] of Object.entries(active.full.providers)) {
    try {
      if (cfg.type === "jsonl") {
        const p = cfg.path || ".memory/data/ltm/store/vectors.jsonl";
        ...
        const res = await fetch(`${cfg.url}/collections/${cfg.collection}`, {
          signal: AbortSignal.timeout(3000),
        });
        results.providers[name] = res.ok
          ? { status: "ok" }
          : { status: "error", error: `${res.status}` };
      } else if (cfg.type === "chroma") {
        const res = await fetch(
          `${cfg.url}/api/v1/collections/${cfg.collection}`,
          { signal: AbortSignal.timeout(3000) },
        );
        results.providers[name] = res.ok
          ? { status: "ok" }
...
if (require.main === module) {
  ...
  (async () => {
    switch (cmd) {
      ...
      default:
        console.log(
          "Commands: query <text> [topK] | ingest (stdin) | status | switch <provider>",
        );
    }
  })().catch((err) => {
