...
function loadExceptions() {
  try {
    ...
    return { version: 1, exceptions: [], trusted_sessions: [] };
  }
}
...
module.exports = function userControlLayer(toolCall, context) {
  ...
  for (const pattern of trustedOps) {
    if (matchOperation(toolCall.tool, toolCall.input, pattern)) {
      return {
        allow: true,
        reason: `Trusted operation: ${pattern}`,
        severity: "info",
  ...
  for (const ex of exceptions.exceptions) {
    if (matchOperation(toolCall.tool, toolCall.input, ex.pattern)) {
      ...
      if (expiresAt > new Date()) {
        return {
          allow: true,
          reason: `User-approved exception: ${ex.reason}`,
          severity: "info",
          layer: "07-user-control",
        };
      }
  ...
  if (isTrustedSession) {
    return {
      allow: true,
      reason: "Trusted session - all operations allowed",
      severity: "info",
      layer: "07-user-control",
    };
  }
  ...
  const currentSessionExceptions = exceptions.exceptions.filter(
