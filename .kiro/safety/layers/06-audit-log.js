...
module.exports = function auditLogLayer(toolCall, context) {
  ...
  if (
    !policy.audit.log_all &&
    !policy.audit.log_blocks &&
    !policy.audit.log_warns
  ) {
    return {
      allow: result.allow,
  ...
  const logPath = policy.audit.log_path || ".memory/data/ltm/store/audit.jsonl";
  const fullPath = path.isAbsolute(logPath)
    ? logPath
    : path.join(PROJ, logPath);
