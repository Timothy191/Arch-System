...
module.exports = function rateLimiterLayer(toolCall, context) {
  ...
  if (fs.existsSync(rateFile)) {
    try {
      state = JSON.parse(fs.readFileSync(rateFile, "utf8"));
    } catch {
      state = { calls: [] };
    }
  }
  ...
  const recentPerTool = state.calls.filter(
    (t) => t.tool === toolCall.tool,
  ).length;
  fs.writeFileSync(rateFile, JSON.stringify(state));

  const limits = policy.rate_limit;
  const toolLimit =
    limits.per_tool_max_per_minute?.[toolCall.tool] ||
    limits.max_calls_per_minute;

  if (recentTotal >= limits.max_calls_per_minute) {
    return {
      allow: false,
      reason: `Rate limit: ${recentTotal}/${limits.max_calls_per_minute} calls/min (all tools)`,
      severity: "block",
      layer: "05-rate-limiter",
  ...
  if (recentPerTool >= toolLimit) {
    return {
      allow: false,
      reason: `Rate limit: ${recentPerTool}/${toolLimit} ${toolCall.tool} calls/min`,
      severity: "block",
      layer: "05-rate-limiter",
      cooldown: limits.cooldown_seconds,
    };
  ...
  if (recentTotal >= limits.max_calls_per_minute * 0.8) {
    return {
      allow: true,
      reason: `Approaching rate limit: ${recentTotal}/${limits.max_calls_per_minute} calls/min`,
      severity: "warn",
      layer: "05-rate-limiter",
    };
  }
  ...
  return {
    allow: true,
    reason: `Rate OK (${recentTotal}/${limits.max_calls_per_minute} calls/min)`,
    severity: "info",
    layer: "05-rate-limiter",
  };
};
