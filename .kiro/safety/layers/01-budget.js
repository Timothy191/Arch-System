...
function loadPolicy() {
  ...
  try {
    ...
    return JSON.parse(
      fs.readFileSync(
        path.join(PROJ, ".kiro", "safety", "policies", "default.json"),
        "utf8",
      ),
    );
  }
...
module.exports = function budgetLayer(toolCall, context) {
  ...
  const toolSpecificFile = path.join(
    getTempDir(),
    `budget-${sessionId}-${toolCall.tool}`,
  );
  ...
  if (fs.existsSync(toolSpecificFile)) {
    toolCount =
      parseInt(fs.readFileSync(toolSpecificFile, "utf8").trim(), 10) || 0;
  }
  toolCount++;
  ...
  if (total >= limits.block_at) {
    return {
      allow: false,
      reason: `Session budget exhausted: ${total}/${limits.block_at} tool calls`,
      severity: "block",
      layer: "01-budget",
    };
  }
  if (toolCount >= toolLimit) {
    return {
      allow: false,
      reason: `Tool ${toolCall.tool} budget exhausted: ${toolCount}/${toolLimit} calls`,
      severity: "block",
      layer: "01-budget",
    };
  }
  if (total >= limits.warn_at) {
    return {
      allow: true,
      reason: `Approaching budget limit: ${total}/${limits.block_at} tool calls`,
      severity: "warn",
      layer: "01-budget",
    };
  }
  ...
  return {
    allow: true,
    reason: `Budget OK (${total}/${limits.block_at})`,
    severity: "info",
    layer: "01-budget",
  };
};
