...
async function main() {
  ...
  for (const layerName of LAYER_ORDER) {
    ...
    try {
      ...
      results.push({
        allow: true,
        reason: `Layer ${layerName} error (fail-open): ${err.message}`,
        severity: "warn",
        layer: layerName,
      });
    }
  ...
  if (!finalDecision.allow) {
    console.error(
      `[safety] BLOCKED by ${finalDecision.layer}: ${finalDecision.reason}`,
    );
  ...
  if (finalDecision.severity === "warn") {
    console.error(
      `[safety] WARN from ${finalDecision.layer}: ${finalDecision.reason}`,
    );
  }
  ...
  if (process.env.SAFETY_DEBUG === "1") {
    for (const r of results) {
      console.error(
        `[safety] ${r.layer}: ${r.allow ? "ALLOW" : "BLOCK"} — ${r.reason}`,
      );
    }
  }
