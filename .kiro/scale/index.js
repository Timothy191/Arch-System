...
function acquireLock(resource, ttlMs = 30000) {
  ...
  try {
    ...
    fs.writeFileSync(
      lockFile,
      JSON.stringify({ acquired_at: Date.now(), owner: process.pid }),
    );
    return true;
  } catch {
    return false;
...
function saveQueue(name, data) {
  ...
  fs.writeFileSync(
    path.join(PROJ, ".kiro", "scale", `queue-${name}.json`),
...
function saveState(data) {
  ...
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(
    path.join(PROJ, ".kiro", "scale", "workers.json"),
    JSON.stringify(data, null, 2),
  );
}
...
function healthCheck() {
  const state = loadState();
  const now = Date.now();
  const stale = state.workers.filter(
    (w) => now - (w.last_heartbeat || 0) > 60000,
  );
...
async function main() {
  ...
  switch (cmd) {
    ...
    case "complete": {
      ...
      if (!taskId) {
        console.error("Usage: scale complete <task_id> [queue]");
        process.exit(1);
      }
    ...
    case "fail": {
      ...
      if (!taskId) {
        console.error("Usage: scale fail <task_id> <error> [queue]");
        process.exit(1);
      }
      ...
      console.log(
        JSON.stringify({
          ...
          remaining: loadQueue(queueName).filter((t) => t.status === "pending")
            .length,
        }),
      );
      break;
    ...
    case "register": {
      ...
      if (!name) {
        console.error("Usage: scale register <name> [capabilities...]");
        process.exit(1);
      }
      const count = registerWorker(name, caps);
      console.log(
        JSON.stringify({ status: "ok", worker: name, total_workers: count }),
      );
      break;
    }
    ...
    case "lock": {
      ...
      if (!resource) {
        console.error("Usage: scale lock <resource> [ttl_ms]");
        process.exit(1);
      }
      const acquired = acquireLock(resource, ttl);
      console.log(
        JSON.stringify({ status: acquired ? "acquired" : "locked", resource }),
      );
      break;
    }
    ...
    case "unlock": {
      const resource = process.argv[3];
      if (!resource) {
        console.error("Usage: scale unlock <resource>");
        process.exit(1);
      }
    ...
    case "status": {
      ...
      try {
        const files = fs
          .readdirSync(path.dirname(QUEUE_FILE))
          .filter((f) => f.startsWith("queue-"));
        for (const f of files) {
          const data = JSON.parse(
            fs.readFileSync(path.join(path.dirname(QUEUE_FILE), f), "utf8"),
          );
          queueFiles.push({
        ...
        JSON.stringify(
          {
            workers: health,
            queues: queueFiles,
            lock_count: fs.existsSync(LOCK_DIR)
              ? fs.readdirSync(LOCK_DIR).length
              : 0,
          },
          null,
          2,
    ...
    default:
      console.log(
        "Commands: enqueue | dequeue | complete | fail | register | assign | lock | unlock | status",
      );
  }
}
