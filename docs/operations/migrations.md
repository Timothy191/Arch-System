...
```
01_platform_packages/database/migrations/
```
...
## Relationship to `01_platform_packages/supabase/migrations/`

`01_platform_packages/supabase/migrations/` exists for Supabase CLI compatibility during local development (`supabase start`, `supabase db push`). However:

- **New migrations must be added to `01_platform_packages/database/migrations/` first.**
- `01_platform_packages/supabase/migrations/` must be kept in sync with `01_platform_packages/database/migrations/` by copying or symlinking.
...
1. Create migration in `01_platform_packages/database/migrations/`
2. Copy to `01_platform_packages/supabase/migrations/` (or run `pnpm policy:migrations:sync`)
3. Apply locally: `pnpm --filter @repo/database supabase:push`
4. Regenerate types: `pnpm --filter @repo/database supabase:gen`
5. Commit migration + regenerated `01_platform_packages/supabase/src/database.types.ts` as one atomic change
6. If `manual-types.ts` changes, commit that too

## Anti-patterns

- ❌ Editing `01_platform_packages/supabase/migrations/` directly without updating `01_platform_packages/database/migrations/`
- ❌ Running `supabase migration new` from the `01_platform_packages/supabase/` directory (it writes to the wrong place)
