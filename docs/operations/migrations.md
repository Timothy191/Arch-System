...
## Relationship to `packages/supabase/migrations/`

`packages/supabase/migrations/` exists for Supabase CLI compatibility during local development (`supabase start`, `supabase db push`). However:

- **New migrations must be added to `packages/database/migrations/` first.**
- `packages/supabase/migrations/` must be kept in sync with `packages/database/migrations/` by copying or symlinking.
...
2. Copy to `packages/supabase/migrations/` (or run `pnpm policy:migrations:sync`)
3. Apply locally: `pnpm --filter @repo/database supabase:push`
4. Regenerate types: `pnpm --filter @repo/database supabase:gen`
5. Commit migration + regenerated `packages/supabase/src/database.types.ts` as one atomic change
6. If `manual-types.ts` changes, commit that too

## Anti-patterns

- ❌ Editing `packages/supabase/migrations/` directly without updating `packages/database/migrations/`
- ❌ Running `supabase migration new` from the `packages/supabase/` directory (it writes to the wrong place)
