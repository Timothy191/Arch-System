---
title: "ADR-008: Turborepo for Monorepo Management"
created: 2026-06-03
updated: 2026-06-03
type: decision
status: superseded
tags: [adr, monorepo, build, decision, superseded]
sources: [turbo.json, package.json, docs/wiki/concepts/adr-011-turborepo-monorepo.md]
confidence: high
---
...
**Superseded** by [[adr-011-turborepo-monorepo]] (September 2026)
...
We will migrate the monorepo task orchestration from **Turborepo** to **Turborepo 2.10.12**, maintaining **pnpm workspaces** for package linkage.

### Key Details

1. **Turborepo Core**: Orchestrate tasks via `turbo.json` and package-specific targets.
2. **Target Defaults Configuration**: Explicitly define dependencies like `build` relying on `^build` and `^codegen`.
3. **Environment Signature Tracking**: Centralize environment variable cache-invalidation signatures in `turbo.json`'s `sharedGlobals`.
4. **Command Redirection**: Route core workspace commands (`pnpm build`, `pnpm lint`, `pnpm test`, `pnpm quality`) through `turbo run`.
...
- **Interactive Graphing**: Ability to generate interactive dependency graphs using `turbo graph` for visual auditing.
- **Topological Integrity**: Native, strict execution order handling for parallel builds.

### Negative

- **Migration Effort**: Required migrating `turbo.json` settings and global scripts to `turbo.json` target defaults.
- **Learning Curve**: Team members must familiarize themselves with `turbo` command flags and caching behaviors instead of `turbo`.
...
- [[turbo-monorepo]] — Updated workspace layout and conventions
