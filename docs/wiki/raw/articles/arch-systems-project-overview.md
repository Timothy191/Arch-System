---
source_url: https://raw.githubusercontent.com/DRACOSFN/Turborepo-Fullstack-Starter-Template/master/pkgs/ui/Template-Fullstack-Starter-Turborepo-v1.1.zip
ingested: 2026-05-15
...
- pkgs/theme/ → @repo/theme — design tokens, CSS variables, Tailwind preset
- pkgs/ui/ → @repo/ui — shared components (GlassCard, DepartmentLayout, KPI, PageHeader, ShiftToggle, FormFields, shadcn primitives)
- pkgs/supabase/ → @repo/supabase — client wrappers (browser, server, middleware) and database types
- pkgs/database/ → @repo/database — SQL migrations (16 migrations, source of truth)
- pkgs/hooks/ → @repo/hooks — useLocalStorage, useDebounce
- pkgs/utils/ → @repo/utils — cn(), formatDate(), getCurrentShift(), excel utilities
- pkgs/eslint-toolchain/ → @repo/eslint-config
- pkgs/typescript-config/ → @repo/typescript-config
...
- Migration source of truth: pkgs/database/migrations/; pkgs/supabase/supabase/migrations/ is a deploy-time copy
