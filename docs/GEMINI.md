...
8. **Codebase-Memory Graph Navigation**:
   - Query `codebase-memory` (`get_architecture`, `search_graph`, `trace_path`) before performing multi-file research to instantly inspect AST symbols, call graphs, and monorepo boundaries without polluting context tokens.
