...
Login and taskbar chrome needed pixel-perfect production polish: crisp white glass inputs, cool ambient lighting, standard placeholders, footer clipping fixes, and taskbar search/tray alignment. Stale layout risk requires all overrides to live under `src/` and sync into `@repo/theme`.

## Decision

1. **`src/01_Admin/styles/portal-production.css`** — production overrides scoped to `.portal-auth-stage` and `.layer-taskbar-brushed`
...
4. Portal wires via `@src/01_Admin/components`; feature barrel re-exports `LoginFormProduction`

## Consequences

### Positive

- Layout changes isolated from legacy `pkgs/theme/glass.css` gold branding
...
- `src/01_Admin/styles/portal-production.css`
