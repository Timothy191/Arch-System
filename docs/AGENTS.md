...
- Explore with `nx-workspace` skill first.
- Run tasks through `nx` (`pnpm nx run`, `pnpm nx run-many`, `pnpm nx affected`) — not underlying tooling directly.
- Prefix with package manager: `pnpm nx build`, not bare `nx`.
- Scaffolding: invoke `nx-generate` skill FIRST.
- Run `node tools/apply-project-tags.cjs` after new projects. Dependency constraints in `nx.json`: `scope:app` → `scope:package`/`scope:feature` only; `scope:package:ui` cannot depend on `scope:package:db`/`db-internal`/`supabase`; `scope:package:theme` cannot depend on `scope:package:ui`.
