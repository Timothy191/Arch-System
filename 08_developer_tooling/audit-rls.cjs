 ...
 * Usage: node 08_developer_tooling/audit-rls.cjs
 */
//
// Static RLS policy audit for 01_platform_packages/database/migrations/*.sql.
//
...
// 08_developer_tooling/circular-dep-detect.cjs.

const fs = require("node:fs");
const path = require("node:path");

const ROOT = path.resolve(__dirname, "..");
const MIGRATIONS_DIR = path.join(ROOT, "01_platform_packages", "database", "migrations");
...
/**
 * Lists all SQL migration files in 01_platform_packages/database/migrations in numeric order.
 *
...
function renderReport({
  ...
  lines.push("");
  lines.push(
    "Generated by `08_developer_tooling/audit-rls.cjs` from `" + path.relative(ROOT, MIGRATIONS_DIR) + "`.",
  );
  lines.push("");
  lines.push("## Summary");
  lines.push("");
